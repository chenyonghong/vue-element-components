...
#### 介绍
本项目基于Vue，对Element-UI常用组件进行二次封装，提供后台管理系统中常用组件的开箱即用。部分组件在相应文件夹下提供了配置文件，稍微改动配置或部分代码即可实现常见业务需求。（另在大中型项目中建议对特定数据进行额外独立组件封装，方便维护和多页面共享）
...
本项目只提供组件的封装使用，不提供后台管理系统模板，请搭配其它后台管理系统模板（如vue-element-admin: [link](https://www.csdn.net/等)使用。    
1.  下载或克隆本项目到本地；
